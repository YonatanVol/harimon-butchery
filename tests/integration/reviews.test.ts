...
import { order, productReview } from "@/infra/db/schema";
...
import { listForModeration, moderateReview, replyToReview, reviewableFor, reviewsFor, submitReview, summariesFor, summaryFor } from "@/infra/reviews/repository";
...
describe("what the shop shows", () => {
  it("shows nothing until a staff member publishes it, then counts it everywhere", async () => {
    ...
    expect(await summariesFor(db, [f.beef.product.id])).toEqual(new Map());
    expect((await listForModeration(db, "PENDING")).map((r) => r.body)).toEqual([BODY]);

    const [pending] = await db.select().from(productReview);
    expect(await moderateReview(db, { id: pending.id, staffId: manager.id, decision: "PUBLISHED", note: null })).toBe(true);
    ...
    expect(await summariesFor(db, [f.beef.product.id])).toEqual(new Map([[f.beef.product.id, { count: 1, averageTenths: 40 }]]));

    // Rejecting takes it straight back off the site.
    expect(await moderateReview(db, { id: pending.id, staffId: manager.id, decision: "REJECTED", note: "  לא על הנתח  " })).toBe(true);
    expect(await reviewsFor(db, f.beef.product.id)).toHaveLength(0);
  ...
  it("keeps the butcher's reply with the review, and lets it be taken back", async () => {
    ...
    expect(await replyToReview(db, { id: saved.id, body: "תודה רבה, נשמח לראותכם שוב." })).toBe(true);
    ...
    expect(await replyToReview(db, { id: saved.id, body: "   " })).toBe(true);
