...
export default async function CheckoutPage({ params, searchParams }: PageProps<"/[locale]/checkout">) {
  ...
  if (!ready) {
    return (
      <div className="mx-auto flex max-w-2xl flex-col items-start gap-4 px-4 py-20 sm:px-6">
        ...
        <h1 className="font-display text-4xl font-light">{t("checkout.title")}</h1>
        <p className="font-reading text-char-700 text-lg">{t("checkout.needCart")}</p>
        <Link href="/cart" className="bg-char-900 text-bone-50 inline-flex min-h-12 items-center rounded-[2px] px-6 font-medium">
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      <h1 className="font-display text-4xl font-light md:text-5xl">{t("checkout.title")}</h1>
      ...
      {pending?.hostedPageUrl && pending.status !== "AUTHORIZED" && pending.status !== "DECLINED" && (
        <div role="status" className="bg-warn-600/10 mt-6 flex flex-wrap items-center justify-between gap-3 rounded-[3px] p-4">
          <div>
          ...
          <a href={pending.hostedPageUrl} className="bg-char-900 text-bone-50 inline-flex min-h-11 items-center rounded-[2px] px-5 text-sm font-medium">
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_380px] lg:items-start">
        ...
        <aside className="bg-bone-100 ring-bone-300 sticky top-32 flex flex-col gap-4 rounded-[3px] p-5 ring-1" aria-labelledby="co-summary">
          <h2 id="co-summary" className="text-lg font-bold">
            {t("checkout.summaryTitle")}
          </h2>
          <div className="bg-bone-50 flex items-center justify-between gap-3 rounded-[2px] p-3">
            <div>
          ...
          <p className="bg-bone-50 text-char-700 font-reading rounded-[2px] p-3 text-sm">
            {quote.hasWeightLines
...
function DeclinedNotice({ title, reason, body }: { title: string; reason: string; body: string }) {
  return (
    <div role="alert" className="border-bad-600 bg-bad-600/10 w-full rounded-[3px] border-s-4 p-4">
